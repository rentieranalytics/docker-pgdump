...
PGPASSWORD="$DESTINATION_PASSWORD" pg_restore -U "$DESTINATION_LOGIN" -h "$DESTINATION_HOST" -F d -v -c --if-exists -j 2 --disable-triggers --no-privileges -d "$DESTINATION_DATABASE" "$DIRECTORY"